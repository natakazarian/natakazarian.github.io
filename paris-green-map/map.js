(() => {
  ...
  const modes = {
    green: {
      ...
      insightTitle: 'Quantity is critically low',
      description: 'The analysis reports a median of 0.68 m² per resident, with many IRIS neighbourhood zones falling in the lowest provision class.',
    ...
    income: {
      ...
      insightTitle: 'The patterns do not align clearly',
      description: 'Income shows a strong west–east spatial gradient, while green-space provision does not reproduce a clear neighbourhood “luxury effect”.',
    ...
    access: {
      ...
      insightTitle: 'Proximity is high',
      description: 'Nearly the entire administrative area of Paris falls within 500 m of a mapped green or assimilated feature.',
  ...
  function insightHtml() {
    const mode = modes[currentMode];
    return `
      <strong>${escapeHtml(mode.insightTitle)}</strong>
      <p>${escapeHtml(mode.description)}</p>
    `;
  }

  function updateLegend() {
    if (legendPanel) legendPanel.innerHTML = legendHtml();
    if (insightPanel) insightPanel.innerHTML = insightHtml();
  }
  ...
  if (legendToggleButton) {
    legendToggleButton.addEventListener('click', () => {
      const isLegendHidden = legendPanel?.classList.toggle('is-hidden');
      if (insightPanel) insightPanel.classList.toggle('is-hidden', isLegendHidden);
      legendToggleButton.classList.toggle('is-active', !isLegendHidden);
      legendToggleButton.setAttribute('aria-expanded', String(!isLegendHidden));
    });
  }

  function toggleFullscreen() {
    const isFS = Boolean(document.fullscreenElement) || mapSection?.classList.contains('is-fullscreen');
    if (!isFS) {
      if (mapSection?.requestFullscreen) {
        mapSection.requestFullscreen().catch(() => mapSection.classList.add('is-fullscreen'));
      } else if (mapSection) {
        mapSection.classList.add('is-fullscreen');
      }
    } else {
      if (document.exitFullscreen && document.fullscreenElement) {
        document.exitFullscreen().catch(() => {});
      }
      if (mapSection) mapSection.classList.remove('is-fullscreen');
    }
  }

  function handleFullscreenChange() {
    const isFS = Boolean(document.fullscreenElement) || (mapSection && mapSection.classList.contains('is-fullscreen'));
