(() => {
  ...
  const story = [
    {
      mode: 'access',
      title: 'Proximity is high',
      text: 'Nearly the entire administrative area of Paris falls within 500 m of a mapped green or assimilated feature.'
    },
    {
      mode: 'green',
      title: 'Quantity is critically low',
      text: 'The accompanying analysis reports a median of 0.68 m² per resident, with many IRIS zones in the lowest class.'
    },
    {
      mode: 'income',
      title: 'The patterns do not align clearly',
      text: 'Income shows a strong west–east gradient, while green-space provision does not reproduce a clear neighbourhood “luxury effect”.'
    }
  ];
  ...
  function setMode(mode, options = {}) {
    ...
    if (!options.fromStory) stopStory();
  }
  ...
  function showStoryStep(index) {
    ...
    setMode(item.mode, { fromStory: true });
    storyCaption.hidden = false;
    storyStep.textContent = `Story ${index + 1} of ${story.length}`;
  ...
  function stopStory(options = {}) {
    if (storyTimer) window.clearInterval(storyTimer);
    storyTimer = null;
    storyButton.classList.remove('is-playing');
    storyButton.textContent = '▶ Play story';
    if (!options.keepCaption) storyCaption.hidden = true;
  }
